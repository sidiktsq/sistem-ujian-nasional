    ...
    <div class="card">
        <form method="POST" action="{{ route('guru.exams.questions.update', [$exam, $question]) }}">
            ...
            <div id="options_container">
                ...
                @php $options = $question->options->sortBy('order'); @endphp
                @for($i = 0; $i < 4; $i++)
                @php $opt = $options->values()[$i] ?? null; @endphp
                <div class="form-group" style="background:rgba(255,255,255,0.02); padding:16px; border-radius:12px; border:1px solid var(--border)">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px">
                        ...
                        <div style="display:flex; align-items:center; gap:8px">
                            <input type="radio" name="correct_option" value="{{ $i }}" id="correct_{{ $i }}" {{ old('correct_option', ($opt && $opt->is_correct ? $i : ($i == 0 && !$question->options->where('is_correct', true)->count() ? 'checked' : ''))) == $i ? 'checked' : ($opt && $opt->is_correct ? 'checked' : '') }}>
                            <label for="correct_{{ $i }}" style="font-size:12px; cursor:pointer; font-weight:600; color:#34D399">Jawaban Benar</label>
                        </div>
                    </div>
                    <input type="text" name="options[]" class="form-control" placeholder="Masukkan teks pilihan..." value="{{ old('options.'.$i, $opt ? $opt->option_text : '') }}">
                </div>
                @endfor
            </div>
